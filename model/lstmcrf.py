...
from model.ee_gcn import GraphConvLayer
from model.ag_gcn import MultiGraphConvLayer, MultiHeadAttention
...
class NNCRF(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.graph_size = config.embedding_dim + config.charlstm_hidden_dim +config.dep_emb_size
        ...
        if self.dep_model != DepModelType.none:
            ...
            self.dep_label_embedding = nn.Embedding(num_embeddings=len(config.deplabel2idx), embedding_dim=config.dep_emb_size,padding_idx=0).to(self.device)
        ...
        if self.dep_model == DepModelType.aelgcn:
            self.pooling = config.pooling
            self.num_gcn_layers = config.num_gcn_layers
            self.gcn_dim = config.gcn_dim
            self.input_W_G = nn.Linear(config.hidden_dim, self.gcn_dim).to(self.device) # lstm output size is config.hidden_dim
            ...
            self.attn = MultiHeadAttention(self.num_att_heads, config.gcn_dim).to(self.device)
            # eegcn
            # self.aggregate_W = nn.Linear(config.gcn_dim + config.num_gcn_layers * config.gcn_dim, final_hidden_dim).to(self.device)
            self.aggregate_W = nn.Linear(len(self.gcn_layers) * config.gcn_dim, final_hidden_dim).to(self.device)
    ...
    def neural_scoring(self, word_seq_tensor, word_seq_lens, batch_context_emb, char_inputs, char_seq_lens, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, dep_head_tensor, dep_label_tensor, pos_label_tensor, trees=None):
        """
        ...
        if self.num_lstm_layer > 0 and self.dep_model != DepModelType.dggcn:
        ...
        """
        Model forward if we have DGLSTM
        """
        if self.num_lstm_layer > 1 and (self.dep_model == DepModelType.dglstm):
            for l in range(self.num_lstm_layer-1):
                ...
                feature_out = self.drop_lstm(lstm_out)
        """
        Model forward if we have DGGCN
        """
        if self.dep_model == DepModelType.dggcn:
            feature_out = self.gcn(feature_out, sorted_seq_len, adj_matrixs[permIdx], dep_label_adj[permIdx])
        """
        Model forward if we have AELGCN
        """
        if self.dep_model == DepModelType.aelgcn:
            dep_label_adj_sorted = dep_label_adj[permIdx]
            edge = dep_label_adj_sorted[:, :sent_len, :sent_len].contiguous()
            weight_adj = self.dep_label_embedding(edge)
            ...
            for _layer in range(len(self.gcn_layers)):
                if _layer % (self.num_blocks + 1) == 0: # EANJU and NAEU 0-th layer, 3-th layer, 6-th layer....
                    ...
                    # AGGCN-Attention guided layer
                    attn_tensor = self.attn(gcn_outputs, gcn_outputs, src_mask)
    ...
    def neg_log_obj(self, words, word_seq_lens, batch_context_emb, chars, char_seq_lens, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, tags, batch_dep_label, batch_pos_label, trees=None):
        ...
        features = self.neural_scoring(words, word_seq_lens, batch_context_emb, chars, char_seq_lens, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, batch_dep_label, batch_pos_label, trees)
    ...
    def decode(self, batchInput):
        ...
        features = self.neural_scoring(wordSeqTensor, wordSeqLengths, batch_context_emb,charSeqTensor,charSeqLengths, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, batch_dep_label, batch_pos_label, trees)
        all_scores = self.calculate_all_scores(features)
