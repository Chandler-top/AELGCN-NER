...
def parse_arguments(parser):
    ...
    parser.add_argument('--num_lstm_layer', type=int, default=1, help="Do not use this flag when tesing our model, this is designed for baselines.")
    parser.add_argument('--dep_emb_size', type=int, default=50, help="embedding size of dependency")
    parser.add_argument('--dep_hidden_dim', type=int, default=200, help="hidden size of dggcn")
    parser.add_argument('--pooling', default='avg', type=str)
    parser.add_argument('--gcn_dim', default=200, type=int, help="hidden size of gcn")
    ...
    parser.add_argument('--num_blocks', type=int, default=2, help='Num of AGGCN blocks.')
    parser.add_argument('--num_sublayers', type=int, default=4, help='Num of the sublayers in dcgcn block.')
    parser.add_argument('--num_att_heads', type=int, default=4, help='Num of heads in AGGCN multi-head attention.')
    ...
    parser.add_argument('--dropout', type=float, default=0.5, help="dropout for lstm embedding")
    parser.add_argument('--use_char_rnn', type=int, default=1, choices=[0, 1], help="use character-level lstm, 0 or 1")
    # parser.add_argument('--use_head', type=int, default=0, choices=[0, 1], help="not use dependency")
    parser.add_argument('--dep_model', type=str, default="aelgcn", choices=["none", "dggcn", "dglstm", "aelgcn"], help="dg_gcn mode consists of both GCN and Syn-LSTM")
...
def learn_from_insts(config:Config, epoch: int, train_insts, dev_insts, test_insts):
    ...
    for i in range(1, epoch + 1):
        ...
        # for index in range(len(batched_data)):
            ...
            loss = model.neg_log_obj(batch_word, batch_wordlen, batch_context_emb,batch_char, batch_charlen, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, batch_label, batch_dep_label, batch_poslabel, trees)
