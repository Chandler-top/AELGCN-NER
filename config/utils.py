...
def simple_batching(config, insts: List[Instance]):
    ...
    poslabel_seq_tensor =  torch.zeros((batch_size, max_seq_len), dtype=torch.long)
    ...
    if config.dep_model != DepModelType.none:
        if  config.dep_model == DepModelType.dggcn or config.dep_model == DepModelType.aelgcn:
            adjs = [ head_to_adj(max_seq_len, inst, config) for inst in batch_data]
            adjs = np.stack(adjs, axis=0)
            adjs = torch.from_numpy(adjs).to(config.device)
            dep_label_adj = [head_to_adj_label(max_seq_len, inst, config) for inst in batch_data]
            dep_label_adj = torch.from_numpy(np.stack(dep_label_adj, axis=0)).long().to(config.device)
        if config.dep_model == DepModelType.dglstm or config.dep_model == DepModelType.dggcn:
            batch_dep_heads = torch.zeros((batch_size, max_seq_len), dtype=torch.long, device=config.device)
            dep_label_tensor = torch.zeros((batch_size, max_seq_len), dtype=torch.long, device=config.device)
    ...
    for idx in range(batch_size):
        ...
        poslabel_seq_tensor[idx, :word_seq_len[idx]] = torch.LongTensor(batch_data[idx].pos_label_ids)
        ...
        # if config.dep_model == DepModelType.aelgcn:
        #     # curr_adj = np.eye(max_seq_len)
        #     curr_dep = np.random.randint(0, 1, (max_seq_len, max_seq_len), dtype=int)
        #     for j,dep_relation in enumerate(batch_data[idx].dep_label_ids):
        #         if j >= max_seq_len:
        #             break
        #         token1_id, token2_id = j, int(batch_data[idx].dep_head_ids[j])
        #         if token2_id == -1 or token2_id >= max_seq_len:
        #             token2_id = token1_id
    ...
    poslabel_seq_tensor = poslabel_seq_tensor.to(config.device)
    ...
    return word_seq_tensor, word_seq_len, word_emb_tensor, char_seq_tensor, char_seq_len, adjs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, trees, label_seq_tensor, dep_label_tensor, poslabel_seq_tensor
